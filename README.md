...
| <a name="input_elastic_version_regex"></a> [elastic\_version\_regex](#input\_elastic\_version\_regex) | The regex of the version of Elasticsearch to use. This will be used in a data object that queries available versions on Elastic<br>    Cloud and will return the most recent version that matches the constraints. This will only be used on the first apply and then<br>    the version will be locked in place so as to not cause unexpected cluster upgrades.<br><br>    CAUTION: Elastic Cloud only supports 3 versions at any one time: the version, the latest version of the previous minor version, and<br>    the latest version of the previous major version. The default is `latest` as this is the safeest way to ensure you will always deploy<br>    with a supported version of Elasticsearch. But you can enter in a regex if you have a specific version you need, just keep in mind that<br>    very few versions are supported. | `string` | `"latest"` | no |
